...
ChatLogic::ChatLogic()
{
    ...
    //_chatBot = new ChatBot("../images/chatbot.png"); // needed???

    // add pointer to chatlogic so that chatbot answers can be passed on to the GUI
    //_chatBot->SetChatLogicHandle(this); // needed???
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        //auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](GraphNode *node) { return node->GetID() == id; }); // Original code
                        ...
                        {
                            //_nodes.emplace_back(new GraphNode(id));   // orgininal code
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            //auto parentNode = std::find_if(_nodes.begin(), _nodes.end(), [&parentToken](GraphNode *node) { return node->GetID() == std::stoi(parentToken->second); });    // original code
                            //auto childNode = std::find_if(_nodes.begin(), _nodes.end(), [&childToken](GraphNode *node) { return node->GetID() == std::stoi(childToken->second); });       // original code
                            ...
                            */
                            ...
                            //(*childNode)->AddEdgeToParentNode(edge);      // original code
                            //(*parentNode)->AddEdgeToChildNode(edge);      // original code
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            if (rootNode == nullptr)
            {
                //rootNode = *it; // assign current node to root // Original code
                rootNode = (*it).get();
            }
            else
            {
    ...
    //rootNode->MoveChatbotHere(_chatBot);  // original code
